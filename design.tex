...
\begin{enumerate}
  ...
  \begin{enumerate}[{1}.1]
    ...
    \CJKindent 图\ref{fig:selectcourse_secquence}是选课用例的时序图
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.8\textwidth]{img/selectcourse_secquence}
      \caption{选课用例时序图}
    ...
    \item 选课用例的类分析与设计
    \begin{itemize}
      \item 边界类：课程列表界面（SelectCourse）。该界面的组件以表单为主，用于显示该学生本学期可选课程，以及获取学生的选课请求。

      \item 控制类：课程管理（CourseManager）。调用getTakes()方法获取已选课信息，调用getCourses()获取可选课程信息。获取学生选课请求之后，调用checkCollision()检测待选课程是否与已选课程存在冲突；若无冲突，则调用take()把选课课程加入到选课信息表中；最后再调用getTakes()和getCourses()更新已选课程列表和可选课程列表。

      \item 实体类：实体类有两个，分别为选课信息（Takes）和课程信息（Course）。前者存储了学生选课的各种信息，包括选课课程信息、学生信息、课程成绩等；后者存储课程信息，包括课程名、开课院系、老师名、开课时间、学分、考查方式、课程类别、教学评分平均成绩及已进行教学评分的人数等等。
    \end{itemize}
  ...
  \begin{enumerate}[{2}.1]
    ...
    \CJKindent 学生可以利用本功能查询个人已选课程的成绩。
    
    \item 查询成绩用例交互过程
    
    \CJKindent 该用例始于学生使用“查询成绩”功能。
    \begin{enumerate}[(1)]
    ...
    \CJKindent 图\ref{fig:query_achievement_sequence}是查询成绩用例的时序图
    \begin{figure}[H]
      \centering
      \includegraphics[width=0.8\textwidth]{img/query_achievement_sequence}
      \caption{查询成绩用例时序图}
    ...
    \item 查询成绩用例的类分析与设计
    \begin{itemize}
      \item 边界类：查询成绩界面（GetScore）。该界面的组件以表单为主，用于获取学生查询某学期成绩的请求，并显示该学生相应学期的课程成绩。
      \item 控制类：成绩管理（ScoreManager）。当获取学生查询某学期成绩的请求之后，调用getTakes()取得该学生相应学期的选课列表信息。通过该列表的条目，先判断该学生是否完成教学评分。若无，则跳转至教学评分界面；否则提取条目中的成绩信息，并调用getCourses()获取该课程名称、学分等信息，最后将这些信息一并显示到查询成绩界面中。
      \item 实体类：实体类有两个，分别为选课信息（Takes）和课程信息（Course）。前者存储了学生选课的各种信息，包括选课课程信息、学生信息、课程成绩等；后者存储课程信息，包括课程名、开课院系、老师名、开课时间、学分、考查方式、课程类别、教学评分平均成绩及已进行教学评分的人数等等。
    \end{itemize}
